...
@pytest.fixture
def device():
    """Fixture for device configuration. Default to CPU."""
    return "cpu"
...
def test_torch_hash_table_serialization(device, sample_keys_torch_large):
    """Test TorchHashTable serialization and deserialization."""
    original_table = TorchHashTable.from_keys(
        sample_keys_torch_large, hash_method=HashMethod.CITY, device=device
    )
    ...
    original_results = original_table.search(sample_keys_torch_large)
    loaded_results = new_table.search(sample_keys_torch_large)
