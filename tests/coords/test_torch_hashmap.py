...
def test_torch_hash_table_creation_and_search(
    device, sample_keys_torch_large, hash_method
):
    """Test TorchHashTable creation, insertion, and search with various hash methods."""
    table = TorchHashTable.from_keys(
        sample_keys_torch_large, hash_method=hash_method, device=device
    )
    assert table.key_dim == sample_keys_torch_large.shape[1]
